...
export default function GestionarCategorias() {
  ...
  const verificarAutenticacion = () => {
    ...
    if (!token) {
      // NUEVO: Notificación antes de redirigir
  ...
  // =================== FUNCIÓN PRINCIPAL MEJORADA CON NOTIFICACIONES ===================
  const cargarCategorias = async (mostrarNotificacionCarga = true) => {
    try {
      setCargando(true);
      setError(null);
      
      // NUEVO: Mostrar notificación de carga
      if (mostrarNotificacionCarga) {
      ...
      if (response.status === 401 || response.status === 403) {
        ...
        setTimeout(() => window.location.href = '/LoginModal', 1500);
        return;
      }
      ...
      // NUEVO: Notificación de error personalizada
  ...
  // =================== FUNCIÓN ELIMINAR MEJORADA CON NOTIFICACIONES ===================
    ...
    try {
      ...
      if (tieneProductos) {
        notificaciones.error(
          ...
          `La ${esSubcategoria ? 'subcategoría' : 'categoría'} "${nombre}" tiene productos asociados. Primero debes eliminar o reasignar los productos.`,
          "⚠️"
        );
        return;
      }
      
      // 3. CONFIRMACIÓN FINAL CON NOTIFICACIÓN PERSONALIZADA
      // Aquí usaríamos un modal de confirmación personalizado en lugar de confirm()
          ...
          <div style={styles.modalContent} onClick={e => e.stopPropagation()}>
            ...
            <div style={styles.modalFooter}>
              <button
                onClick={() => {
                  cerrarModal();
                  notificaciones.info("Creación cancelada", "La creación de categoría ha sido cancelada", "❌");
                }}
...
// Mantener los estilos exactamente iguales...
const styles = {
  container: {
    padding: '24px',
    maxWidth: '1400px',
